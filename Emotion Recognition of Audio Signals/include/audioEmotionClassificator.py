...
class ERAS(object):
    ...
    def __dataPreProcessing(dataset, categories, scaler = StandardScaler(), test_size = 0.2):
    ...
    def __modelScore(categories, model, X_test, y_test):
            
        prediction = model.predict(X_test)

        print(classification_report(prediction, y_test, target_names = categories))
        df_cm = pd.DataFrame(confusion_matrix(prediction, y_test, normalize='true'), index = [i for i in categories],
    ...
    def trainModel(self, dataset, categories = "Angry Calm Dark Dramatic Happy Inspirational Romantic Sad Bright", test_size = 0.2, model = KNeighborsClassifier(n_neighbors=2), showScore = True, scaler=StandardScaler()):
        
        categories = categories.split()
        
        X_train, X_test, y_train, y_test, scaler, encoder = ERAS.__dataPreProcessing(dataset, categories, scaler)
        ...
        if(showScore):
           ERAS.__modelScore(categories, model, X_test, y_test)
    ...
    def predict(self, track, offset = None, duration = None, sr = 22050, window_seconds = 15, hop_seconds = 10, transcribe = False):
        ...
        features = af.features_extractor_windowed(track = y, sr = sr, mfccs_n = 40, window_seconds = window_seconds, hop_seconds = hop_seconds)
        ...
        img = df['Emotions'].value_counts().plot.pie(labels = prediction.index, colors=[colors[key] for key in prediction.index], ax=ax[0])
        ...
        for row in df.iterrows():
            ...
            x1 = x + hop_seconds
            plt.fill_between([x,x1], 1,y2=15, color=colors[row[1]['Emotions']])
